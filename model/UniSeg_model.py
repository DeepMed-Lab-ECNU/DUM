...
# from Unet import DownTransition, UpTransition, OutputTransition
...
class UniSeg_model(nn.Module):
    def __init__(self, img_size, in_channels, out_channels, task_total_number=32, act='relu', encoding='rand_embedding'):
        ...
        conv_kwargs = None#{'stride': 1, 'dilation': 1, 'bias': True,  'kernel_size': [(3, 3, 3)] * (num_conv_stage + 1), '':}
        conv_op = nn.Conv3d
        ...
        self.intermedia_prompt = nn.Parameter(torch.randn(1, task_total_number, img_size[0]//8, img_size[1]//8, img_size[2]//8))
        self.fusion_layer = StackedFusionConvLayers(final_num_features+task_total_number, (final_num_features+task_total_number)//4, task_total_number, num_conv_stage,
                                                              conv_op, conv_kwargs, norm_op,
                                                              norm_op_kwargs, dropout_op,
                                                              dropout_op_kwargs, nonlin, nonlin_kwargs,
                                                              None)

        self.up_tr256 = UpTransition(512, 512,2,act, 1)
    ...
    def convert_task_id(self, tasks_id, device):
        tasks_id = torch.tensor(int(tasks_id[0].split('/')[0].split('_')[0])).to(device)
        return tasks_id

    def forward(self, x_in, tasks_id):

        tasks_id = self.convert_task_id(tasks_id, x_in.device)
        bs = x_in.shape[0]
        ############################## encoder #################################
        self.out64, self.skip_out64 = self.down_tr64(x_in)
        self.out128,self.skip_out128 = self.down_tr128(self.out64)
        self.out256,self.skip_out256 = self.down_tr256(self.out128)
        self.out512,self.skip_out512 = self.down_tr512(self.out256)
        ############################## fusion prompt #################################
        now_prompt = self.intermedia_prompt.repeat(bs,1,1,1,1)
        dynamic_prompt = self.fusion_layer(torch.cat([self.out512, now_prompt], dim=1))
        task_prompt = torch.index_select(dynamic_prompt, 1, tasks_id)
        self.out512 = torch.cat([self.out512, task_prompt], dim=1)
        ############################## decoder #################################
...
class UniSeg_model_Standard(Generic_UNet):
    def __init__(self, patch_size=[96, 96, 96], task_total_number=32, input_channels=1, base_num_features=32, num_classes=32, num_pool=5, num_conv_per_stage=2,
    ...
    def forward(self, x, tasks_id):
        ...
        bs = x.size()[0]
        # x = x[:,0]
        # print(f" orignal x shape is {x.shape} and {self.conv_blocks_context[0](x)}")

        for d in range(len(self.conv_blocks_context) - 1):
            # print(x.size())
            x = self.conv_blocks_context[d](x)
            skips.append(x)
            # print(f"middle x shape is {x.shape}")
        ...
        now_prompt = self.intermedia_prompt.repeat(bs,1,1,1,1)
        # print(x.shape, now_prompt.shape)
        dynamic_prompt = self.fusion_layer(torch.cat([x, now_prompt], dim=1))
        tasks_id = self.convert_task_id(tasks_id, x.device)
        task_prompt = torch.index_select(dynamic_prompt, 1, tasks_id)
...
if __name__ == "__main__":
    model = UniSeg_model_Standard(input_channels=[1,1,1], num_classes=32)
    input_ = torch.rand((4, 1, 96, 96, 96))
    out = model(input_, '05_KiTS')
