...
class ResNet(nn.Module):
    ...
    def forward(self, x):
        ...
        # x[1]: [batch_size, 1]
        x0 = self.stem(x)  # [batch_size, 64, 56, 56]
        x1 = self.layer1(x0)
        x2 = self.layer2(x1)
        x3 = self.layer3(x2)
        x4 = self.layer4(x3)  # [batch_size, 2048, 7, 7]
        x5 = self.avgpool(x4)  # [batch_size, 2048, 1, 1]
        x5 = torch.flatten(x5, 1)  # [batch_size, 2048*1*1]
        x5 = self.head(x5)  # [batch_size, 1024]
...
class ModifiedResNet(nn.Module):
    ...
    def __init__(self, layers, output_dim, heads=8, image_size=224, width=64):
        super().__init__()
        self.output_dim = output_dim
        self.image_size = image_size
...
class ModifiedResNetV2(nn.Module):
    ...
    def forward_features(self, x):
        x0 = self.stem(x)  # [batch_size, 64, 112, 112]
        x1 = self.avgpool(x0) #
        x1 = self.layer1(x1) # [batch_size, dim, 56, 56]
        x2 = self.layer2(x1) # [batch_size, dim, 28, 28]
