...
class DiceLoss(nn.Module):
    ...
    def forward(self, predict, target, name, TEMPLATE, use_sigmoid=True):
        
        total_loss = []
        predict = F.sigmoid(predict) if use_sigmoid else predict
...
class Multi_BCELoss(nn.Module):
    def __init__(self, ignore_index=None, num_classes=3, withlogit=True, **kwargs):
        super(Multi_BCELoss, self).__init__()
        self.kwargs = kwargs
        self.num_classes = num_classes
        self.ignore_index = ignore_index
        self.criterion = nn.BCEWithLogitsLoss() if withlogit else nn.BCELoss()
